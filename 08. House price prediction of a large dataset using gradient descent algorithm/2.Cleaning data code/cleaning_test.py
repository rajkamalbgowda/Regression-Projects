# -*- coding: utf-8 -*-
"""
Created on Thu Dec 19 14:26:38 2019
...
df=pd.read_csv("C:/Users/RANVI/Desktop/Silfra assignment- Rajkamal/1.Original data/test.csv")
...
#2 MSZoning
df_eda['MSZoning']
df_eda['MSZoning'].unique()
df_eda['MSZoning'].describe()

                                      #categorical and non numerical

df_eda['MSZoning'].isnull().sum() #no null values
df_eda['MSZoning'] = df_eda['MSZoning'].fillna((df_eda['MSZoning'].mode()[0])) 

plt.hist(df_eda['MSZoning'])
...
df_eda['Utilities'] = df_eda['Utilities'].fillna((df_eda['Utilities'].mode()[0])) 
df_eda['Utilities'].isnull().sum()
...
del dfDummies_RoofMatl['RoofMatl_CompShg'] #delete dummy
...
df_eda['Exterior1st'].count() #catogorical and non numerical

df_eda['Exterior1st'].isnull().sum()
df_eda['Exterior1st'] = df_eda['Exterior1st'].fillna((df_eda['Exterior1st'].mode()[0])) 
df_eda['Exterior1st'].isnull().sum()


plt.hist(df_eda['Exterior1st']) 
...
df_eda['Exterior2nd'].unique() #catogorical and non numerical
df_eda['Exterior2nd'].describe()
df_eda['Exterior2nd'].count()

df_eda['Exterior2nd'].isnull().sum()
df_eda['Exterior2nd'] = df_eda['Exterior2nd'].fillna((df_eda['Exterior2nd'].mode()[0])) 
df_eda['Exterior2nd'].isnull().sum()


plt.hist(df_eda['Exterior2nd']) 
...
del dfDummies_Heating['Heating_GasA'] #delete dummy
...
del dfDummies_BsmtFullBath['BsmtFullBath_0.0'] #del one dummy variable
...
del dfDummies_BsmtHalfBath['BsmtHalfBath_0.0'] #del one dummy variable
...
df_eda['KitchenQual'].describe() #categorical and non numerical

df_eda['KitchenQual'].count()

df_eda['KitchenQual'].isnull().sum()
df_eda['KitchenQual'] = df_eda['KitchenQual'].fillna((df_eda['KitchenQual'].mode()[0])) 
df_eda['KitchenQual'].isnull().sum()

plt.hist(df_eda['KitchenQual'])
...
df_eda['Functional'].unique()#categorical and non numerical
df_eda['Functional'].describe()
df_eda['Functional'].count()


df_eda['Functional'].isnull().sum()
df_eda['Functional'] = df_eda['Functional'].fillna((df_eda['Functional'].mode()[0])) 
df_eda['Functional'].isnull().sum()

plt.hist(df_eda['Functional'])
...
del dfDummies_GarageCars['GarageCars_1.0'] #del one dummy variable
...
del dfDummies_GarageQual['GarageQual_Fa']
...
df_eda['SaleType'].describe()  #categorical and non numerical
df_eda['SaleType'].count()
df_eda['SaleType'].unique()


df_eda['SaleType'].isnull().sum()
df_eda['SaleType'] = df_eda['SaleType'].fillna((df_eda['SaleType'].mode()[0])) 
df_eda['SaleType'].isnull().sum()

plt.hist(df_eda['SaleType'])
...
data_conc.to_csv("C:/Users/RANVI/Desktop/Silfra assignment- Rajkamal/3.Cleaned output/cleaned_test_data.csv',index=False)

cleaned_test_data=pd.read_csv("C:/Users/RANVI/Desktop/Silfra assignment- Rajkamal/3.Cleaned output/cleaned_test_data.csv')
